...
exports.addOrder = async (req, res) => {
	...
	const foundchef = await User.updateMany({
...
exports.showOrder = async (req, res) => {
	...
	const foundOrders = await Order.find({})
		.populate({ path: 'productName', populate: 'chef' })
		.exec();
...
exports.addOrdertoChef = async (req, res) => {
	...
	const removeOrder = await User.updateMany({
		$pull: { orders: { $in: [newOrder] } }
	}).exec();
	const chefearning = await User.findOne({ _id: chefid });
	const foundchef = await User.findOneAndUpdate(
		{ _id: chefid },
		{
			$push: { orders: newOrder },
			earnings: chefearning.earnings + newOrder.price
		}
	).exec();
